...
- `taistamp seed new [selector]` — mints a fresh
  ...
  and prints the `<selector>._taistamp` DNS TXT record
  publishing its public half. The selector defaults to
  `default`; rotate by passing a real selector name
  (e.g. `s1`).
- `taistamp probe <url>` — fetches `<url>/.well-known/taistamp`
  (`https://` assumed when no scheme is given; the path
  is smart-appended when `<url>` is a bare origin) with
  a fresh nonce, then either:
...
All commands accept `--env-file <path>` (after the
subcommand) to load environment variables from a
dotenv-style file — e.g. wrangler's `.dev.vars` — and
`./.env` is auto-loaded when present. File values never
